...
import { HistoryProps, TemplateProps } from "@/types";
...
async function HistoryPage() {
  ...
  const getTemplateName = (slug: string) => {
    const template: TemplateProps | any = Templates?.find((item) => item.slug == slug);
    return template;
  };
  ...
  return (
    <div className="m-5 p-5 border rounded-lg bg-white">
      ...
      {HistoryList.map((item: HistoryProps, index: number) => (
        <>
